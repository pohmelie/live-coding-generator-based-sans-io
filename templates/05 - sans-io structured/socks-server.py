...
def write_struct(fmt, *args):
    s = struct.Struct(fmt)
    bin = s.pack(*args)
    yield from write(bin)

def protocol():
    fmt = "!BBH4s"
    version, cmd, port, ipv4 = yield from read_struct(fmt)
    assert version == 4
    assert cmd == 1
    identity = yield from read_c_string()
    ...
    try:
        ...
        print("error", e)
        yield from write_struct(fmt, 0, 0x5b, 0, b"\x00" * 4)
    else:
        yield from write_struct(fmt, 0, 0x5a, 0, b"\x00" * 4)
